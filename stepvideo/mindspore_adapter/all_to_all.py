...
class SeqAllToAll4D(nn.Cell):
    ...
    def construct(self, input: Tensor, scatter_idx: int, gather_idx: int) -> Tensor:
        ...
        if scatter_idx == 2 and gather_idx == 1:
            ...
            input_t = (
                input.reshape(bs, shard_seqlen, self.sp_size, shard_hc, hs)
                .swapaxes(0, 2)
                .contiguous()
            )
    ...
    def bprop(self, x: Tensor, scatter_idx: int, gather_idx: int, out: Tensor, dout: Tensor) -> tuple[Tensor, None, None]:
        return (
